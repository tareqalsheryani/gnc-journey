...
Result: ZN gains failed to stabilize the pole. The response drifted slowly and never settled within the 5s window. Response drifted till -0.085 rad. The hand tuned PI controller (Kp=25, Ki=1.8) outperformed the ZN one, achieving the textbook classic overshoot then lock to zero.
